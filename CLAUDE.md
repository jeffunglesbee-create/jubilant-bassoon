...
6. **ADR-002 / RUWT compliance** — READ `docs/ADR-002-CONTEXT.md` before any compliance audit. Contains full defense architecture, mitigations, amnesty zone, and severity classification. Many patterns that look like violations are already mitigated. Rule F (added 2026-07-20) clarifies the commodity/proprietary test: the relay may compute industry-standard statistical models (win probability, xG, Elo); it may never compute outputs that function as watch recommendations regardless of label or data type.
