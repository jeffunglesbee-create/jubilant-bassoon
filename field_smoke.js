...
// ── Assertions 56-65: Drama Engine + ViewingConditions ──────────────────────
if(html.includes('dramaScoreLive') && /function dramaScoreLive\s*\(/.test(html))
  pass('Assertion 56 — drama-score-live: dramaScoreLive present');
else fail('Assertion 56 — drama-score-live: dramaScoreLive missing');

if(html.includes('SPORT_CRUNCH_RULES') && html.includes('isCrunchTime'))
  pass('Assertion 57 — situation-drama-bonus: SPORT_CRUNCH_RULES + isCrunchTime present');
else fail('Assertion 57 — situation-drama-bonus: missing SPORT_CRUNCH_RULES or isCrunchTime');

if(/function getStatisticalExtremes\s*\(/.test(html))
  pass('Assertion 58 — statistical-extreme-notes: getStatisticalExtremes defined');
else fail('Assertion 58 — statistical-extreme-notes: getStatisticalExtremes missing');

if(html.includes('ViewingConditions') && html.includes("'CRUNCH TIME'"))
  pass("Assertion 59 — b1plus-crunch-time: ViewingConditions + 'CRUNCH TIME' present");
else fail("Assertion 59 — b1plus-crunch-time: ViewingConditions or 'CRUNCH TIME' missing");

if(html.includes('isFinalPeriod') && html.includes('Math.min(score, 20)'))
  pass('Assertion 60 — b1plus-garbage-time: isFinalPeriod + garbage-time cap present');
else fail('Assertion 60 — b1plus-garbage-time: isFinalPeriod or garbage-time cap missing');

if(html.includes('isBlowout') && html.includes('ViewingConditions'))
  pass('Assertion 61 — b1plus-j5-isblowout: isBlowout in ViewingConditions present');
else fail('Assertion 61 — b1plus-j5-isblowout: isBlowout or ViewingConditions missing');

if(/function applyCardPulse\s*\(/.test(html) && html.includes('badge-pulse'))
  pass('Assertion 62 — b1plus-pulse: applyCardPulse + badge-pulse present');
else fail('Assertion 62 — b1plus-pulse: applyCardPulse or badge-pulse missing');

if(html.includes('wxCache') && html.includes('wx?.alert') && html.includes('sitBonus'))
  pass('Assertion 63 — weather-drama-bonus: wxCache + weather alert bonus present');
else fail('Assertion 63 — weather-drama-bonus: weather bonus pattern missing');

if(html.includes('[EMBER BURIED LEAD') && html.includes('_emberResultCache'))
  pass('Assertion 64 — ember-fields-compound: EMBER BURIED LEAD in compound prompt');
else fail('Assertion 64 — ember-fields-compound: EMBER BURIED LEAD missing from prompt');

if(html.includes('FRANCHISE_MISERY') && /function getFranchiseMisery\s*\(/.test(html))
  pass('Assertion 65 — franchise-misery-index: FRANCHISE_MISERY + getFranchiseMisery present');
else fail('Assertion 65 — franchise-misery-index: missing');

// ── Assertions 66-74: J-Series Journalism ───────────────────────────────────
if(html.includes('anti-hype') && html.includes('anti-hype-badge'))
  pass('Assertion 66 — j1-anti-hype-cards: anti-hype + anti-hype-badge present');
else fail('Assertion 66 — j1-anti-hype-cards: anti-hype pattern missing');

if(/function buildSeriesPreviewStatic\s*\(/.test(html) && html.includes('series-preview-card'))
  pass('Assertion 67 — j2-series-preview: buildSeriesPreviewStatic + series-preview-card');
else fail('Assertion 67 — j2-series-preview: missing');

if(/function initFIELDBrief\s*\(/.test(html) && html.includes('field-brief'))
  pass('Assertion 68 — j3-field-brief: initFIELDBrief + field-brief present');
else fail('Assertion 68 — j3-field-brief: missing');

if(html.includes('DRAMA: RISING') && html.includes('DRAMA: COOLING'))
  pass('Assertion 69 — j3-momentum-briefs: DRAMA: RISING + COOLING in compound prompt');
else fail('Assertion 69 — j3-momentum-briefs: DRAMA: RISING or COOLING missing');

if(html.includes('scout-pick-badge') && html.includes("Scout\'s Pick"))
  pass("Assertion 70 — j4-scouts-pick: scout-pick-badge + Scout's Pick present");
else fail("Assertion 70 — j4-scouts-pick: scout-pick-badge missing");

if(html.includes('scout-pick-card') && html.includes('.scout-pick-card'))
  pass('Assertion 71 — j4-scouts-pick-cards: scout-pick-card class present');
else fail('Assertion 71 — j4-scouts-pick-cards: scout-pick-card missing');

if(/function renderNightOwlRecap\s*\(/.test(html) && html.includes('night-owl-card'))
  pass('Assertion 72 — j5-night-owl: renderNightOwlRecap + night-owl-card present');
else fail('Assertion 72 — j5-night-owl: missing');

if(html.includes('const MEDIA_SPECIALS') && html.includes('MEDIA_SPECIALS'))
  pass('Assertion 73 — j6-smt-notes: MEDIA_SPECIALS array present');
else fail('Assertion 73 — j6-smt-notes: MEDIA_SPECIALS missing');

if(html.includes('cdw-notes') && html.includes('matchupNote'))
  pass('Assertion 74 — j7-cdw-notes: cdw-notes + matchupNote present');
else fail('Assertion 74 — j7-cdw-notes: missing');

// ── Assertions 75-82: Night Owl + BDL + AFL + ATP ───────────────────────────
if(/function getDramaPeakWithTime\s*\(/.test(html))
  pass('Assertion 75 — night-owl-peak-time: getDramaPeakWithTime defined');
else fail('Assertion 75 — night-owl-peak-time: getDramaPeakWithTime missing');

if(/function getDramaSustained\s*\(/.test(html))
  pass('Assertion 76 — night-owl-sustained-drama: getDramaSustained defined');
else fail('Assertion 76 — night-owl-sustained-drama: getDramaSustained missing');

if(/function refreshAFLSection\s*\(/.test(html) && html.includes('_aflDramaScore'))
  pass('Assertion 77 — afl-drama-v5: refreshAFLSection + _aflDramaScore present');
else fail('Assertion 77 — afl-drama-v5: missing');

if(/function fetchATPLiveScores\s*\(/.test(html) && html.includes('atpIntervalId'))
  pass('Assertion 78 — atp-live-scores: fetchATPLiveScores + atpIntervalId present');
else fail('Assertion 78 — atp-live-scores: missing');

if(/function bdlInjuryContextSync\s*\(/.test(html))
  pass('Assertion 79 — bdl-injury-compound: bdlInjuryContextSync defined');
else fail('Assertion 79 — bdl-injury-compound: bdlInjuryContextSync missing');

if(html.includes('_bdlMilestonesCache'))
  pass('Assertion 80 — bdl-milestones-p2c: _bdlMilestonesCache present');
else fail('Assertion 80 — bdl-milestones-p2c: _bdlMilestonesCache missing');

if(/function getBNIStrength\s*\(/.test(html) && /function computeBroadcastNarrativeIndex\s*\(/.test(html))
  pass('Assertion 81 — bni-strength-otw: getBNIStrength + computeBroadcastNarrativeIndex present');
else fail('Assertion 81 — bni-strength-otw: missing');

if(/function detectArcType\s*\(/.test(html))
  pass('Assertion 82 — detect-arc-type: detectArcType defined');
else fail('Assertion 82 — detect-arc-type: detectArcType missing');

// ── Assertions 83-89: Live Polling + ESPN Infra ──────────────────────────────
if(html.includes('halftime-switch') && html.includes('_quietHourActive'))
  pass('Assertion 83 — halftime-switch-polling: halftime-switch + _quietHourActive present');
else fail('Assertion 83 — halftime-switch-polling: missing');

if(html.includes('_quietHourActive') && html.includes('QUIET_CYCLES_GATE'))
  pass('Assertion 84 — quiet-hour-detection: _quietHourActive + QUIET_CYCLES_GATE present');
else fail('Assertion 84 — quiet-hour-detection: missing');

if(html.includes('_espnLeagueState') && html.includes('leaguesToPoll'))
  pass('Assertion 85 — per-league-polling: _espnLeagueState + leaguesToPoll present');
else fail('Assertion 85 — per-league-polling: missing');

if(html.includes('leaguesToPoll') && html.includes('Promise.all'))
  pass('Assertion 86 — concurrent-espn-fetch: leaguesToPoll + Promise.all present');
else fail('Assertion 86 — concurrent-espn-fetch: missing');

if(html.includes('homeLinescores') && html.includes('awayLinescores'))
  pass('Assertion 87 — period-score-extraction: homeLinescores + awayLinescores present');
else fail('Assertion 87 — period-score-extraction: missing');

if(/function renderScoreTicker\s*\(/.test(html))
  pass('Assertion 88 — ticker-trend-sort: renderScoreTicker defined');
else fail('Assertion 88 — ticker-trend-sort: renderScoreTicker missing');

if(/function fieldBriefCacheKey\s*\(/.test(html) && html.includes('field_brief_'))
  pass('Assertion 89 — qw7-briefcache: fieldBriefCacheKey + field_brief_ key present');
else fail('Assertion 89 — qw7-briefcache: missing');

// ── Assertions 90-92: Relay Constants ────────────────────────────────────────
if(html.includes('FD_RELAY'))
  pass('Assertion 90 — relay-fd: FD_RELAY present');
else fail('Assertion 90 — relay-fd: FD_RELAY missing');

if(html.includes('FPL_RELAY_BASE'))
  pass('Assertion 91 — relay-fpl: FPL_RELAY_BASE present');
else fail('Assertion 91 — relay-fpl: FPL_RELAY_BASE missing');

if(html.includes('NHL_RELAY_BASE'))
  pass('Assertion 92 — relay-nhl: NHL_RELAY_BASE present');
else fail('Assertion 92 — relay-nhl: NHL_RELAY_BASE missing');

// ── Assertions 93-101: Misc Features ─────────────────────────────────────────
if(/function fetchMLBSchedule\s*\(/.test(html) && html.includes('MLB_STATS_BASE'))
  pass('Assertion 93 — mlb-stats-adapter: fetchMLBSchedule + MLB_STATS_BASE present');
else fail('Assertion 93 — mlb-stats-adapter: missing');

if(/function fetchMLBLiveGame\s*\(/.test(html) && html.includes('feed/live'))
  pass('Assertion 94 — mlb-thin-client: fetchMLBLiveGame + feed/live endpoint present');
else fail('Assertion 94 — mlb-thin-client: missing');

if(/function buildMyTeamsBriefLine\s*\(/.test(html))
  pass('Assertion 95 — my-teams-brief: buildMyTeamsBriefLine defined');
else fail('Assertion 95 — my-teams-brief: buildMyTeamsBriefLine missing');

if(/function buildLayer3Rules\s*\(/.test(html))
  pass('Assertion 96 — layer3-templates: buildLayer3Rules defined');
else fail('Assertion 96 — layer3-templates: buildLayer3Rules missing');

if(/function updatePinWidget\s*\(/.test(html) && html.includes('_pinWidgetExpanded'))
  pass('Assertion 97 — card-mini-dashboard: updatePinWidget + _pinWidgetExpanded present');
else fail('Assertion 97 — card-mini-dashboard: missing');

if(html.includes('BEGIN:VCALENDAR') && html.includes('.ics'))
  pass('Assertion 98 — ics-export: BEGIN:VCALENDAR + .ics present');
else fail('Assertion 98 — ics-export: missing');

if(html.includes('SW_VERSION') && html.includes('?v=${SW_VERSION}'))
  pass('Assertion 99 — sw-version-cache-bust: SW_VERSION cache-bust query param present');
else fail('Assertion 99 — sw-version-cache-bust: missing');

const fs_check = require('fs');
const path_check = require('path');
const _preCommitPath = path_check.join(__dirname, 'scripts/pre-commit');
const _repoPreCommit = '/tmp/jubilant-bassoon/scripts/pre-commit';
if(fs_check.existsSync(_preCommitPath) || fs_check.existsSync(_repoPreCommit))
  pass('Assertion 100 — auto-deploy-guard-v5: scripts/pre-commit smoke gate present');
else fail('Assertion 100 — auto-deploy-guard-v5: scripts/pre-commit missing');

if(/function fieldVsMarket\s*\(/.test(html) && html.includes('scoutTitle'))
  pass('Assertion 101 — scout-pick-market-tip: fieldVsMarket + scoutTitle enrichment present');
else fail('Assertion 101 — scout-pick-market-tip: missing');
  ...
  if (failures > 0) {
    ...
    console.log(`SMOKE TEST PASSED (${sportSections} sport sections — 101 assertions: structural+drama+journalism+relay+polling+misc verified)`);
    process.exit(0);
  }
})();
