# CC-CMD: Update ADR-002 Rules A, B, C — permit relay-side scalar computation under a pull-only guardrail
...
## PROBE BLOCK
```bash
sed -n '113,145p' docs/ADR-002-CONTEXT.md
```
Confirm all three rules' text still matches the citations below before
editing.

## THE CORRECTION, STATED ONCE, APPLIED TO ALL THREE RULES
...
- `git diff` after editing — confirm exactly three hunks changed (Rules
  A, B, C), nothing else in the file touched, matching the pattern
...
- Re-read all three new rules together and confirm they're mutually
  ...
  covered) — not contradictory or redundant with each other.
...
- [ ] Exactly three hunks in the diff, nothing else touched
...
+25  Rule A replaced correctly
+25  Rule B replaced correctly
+25  Rule C replaced correctly, explicitly states no code migration happened
+15  Rule D confirmed untouched, diff is exactly three hunks
...
Rules A, B, and C exactly as specified -- all three permit relay-side
scalar computation under a pull-only guardrail, correcting text that
was over-broad relative to the actual patent claims. Rule D is
unchanged and must stay that way. This is documentation only -- no code
migration happens in this CC-CMD, state that explicitly in the outbox,
and explicitly distinguish the legal justification (corrected patent
reading) from the cost-feasibility fact (25.1ms/month), per Rule D's
own point about not conflating the two. Do not commit unless confidence
