...
**A second, equally real precondition, found via direct smoke.js inspection — not anticipated in this spec's first draft:** two existing regression tests, `A346` (JQ-INFRA-3, catches a real May 31 silent-`ReferenceError` production bug) and `A347` (JQ-INFRA-2, catches a real May 31 function-hoisting scope-trap bug), both re-extract the script body from `index.html` and re-execute it via `new Function(checkSrc)()` — a classic-function constructor that **cannot execute code containing real `export` statements at all**. This is independent of whatever the outer `<script>` tag's type is; `new Function()` never runs in the browser's real module-loading context in the first place. Adding real `export` syntax to `field.js` will make both of these tests throw a `SyntaxError` immediately, not fail their actual check — a different, more basic kind of break than either test currently guards against.

**These two tests exist because of real, previously-occurred, silent production bugs — do not disable or weaken them to work around this.** The correct fix is adapting their own extraction/execution mechanism to handle real module syntax (e.g., stripping `export` keywords before the `new Function()` re-execution, since the tests only care about scope/hoisting behavior, not the module system itself — confirm this approach is sound, don't assume it without checking what `export function X(){}` vs `function X(){}` actually changes for the specific `typeof`/identifier checks these tests perform).
...
5. **A346/A347 compatibility:** confirm the real, current behavior of both tests' `new Function()`-based re-execution once `export` syntax is added — test this directly with a small, real, isolated example before touching the full 25-function conversion, and confirm whether stripping `export` keywords for the purposes of these two tests' own extraction (not the real deployed code) is a safe, correct fix, or whether a different approach is needed.
