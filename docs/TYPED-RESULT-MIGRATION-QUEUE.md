...
| 4 | `fetchGameBriefOnDemand` (~L32289) | 1 | ✅ MIGRATED 2026-07-13 (CC-CMD-bucketb-tierc-cluster3): this function has 2 real catches. Added `captureFieldError('journalism:game-brief-on-demand', ...)` to the generic NBA/NHL/other branch's generation catch. **A second candidate site (championship-archive enrichment) was initially instrumented then REVERTED** — cross-checking against the full Bucket C list found it's already classified there (~L735: "null-safe, returns ctx unchanged on any failure"), confirming the CC-CMD's own explicit "never touch Bucket C" boundary. **Real bug found and fixed via this same investigation:** `budget` was declared `const` inside `if (!usesOwnBudget) {...}`, but referenced via `budget.inc()` in the generic branch well outside that block — a genuine `ReferenceError` on every successful generic-branch generation, previously invisible because it landed in the (until-now-silent) catch. Hoisted the declaration to function scope; `journalismCallsToday()` itself is a pure factory (safe to call unconditionally). Forced-tested including a direct proof that a genuine success now reaches `budget.inc()` without throwing. Zero *intended* caller behavior change (the bug fix is a correctness restoration, not a new behavior). |
| 4 | `fetchEPLMatchBriefFromClaude` (~L32421) | 1 | ✅ MIGRATED 2026-07-13 (CC-CMD-bucketb-tierc-cluster3): this function has 2 real catches. Added `captureFieldError('journalism:epl-brief', ...)` to the main generation catch. **A second candidate site (H2H/form-context builder) was initially instrumented then REVERTED** — cross-checking against the full Bucket C list found it's already classified there (~L736: "Optional H2H/form context builder; catch just leaves strings blank and execution continues into the prompt"), confirming the CC-CMD's own explicit "never touch Bucket C" boundary. Forced-tested both (including proving the context-build catch correctly stays silent). Zero caller behavior change. |
