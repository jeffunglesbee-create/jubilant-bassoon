...
| B — decorative only (telemetry gap) | 281 | 34.0% |
| C — legitimately fine as-is | 519 | 62.8% |
...
| 6 | `fetchStakesBriefFromClaude` (~L31946) | 1 | renderStakesBriefCard's if/else treats budget-exceeded identically to every other null cause (card.remove()). |
...
| 5 | `_resolveRealGameId` (~L10646) | 5 | All 5 callers store result directly as `_gameId:` field, never branch on it; downstream fuzzy-match fallback is identical for every failure reason |
| 5 | `generateJournalismViaRelay` (~L17545) | 5 | Bad-prompt guard collapses into the same null all 5 callers treat identically via `if(_viaRelay&&_viaRelay.length>30){...} else {legacy fallback}`. |
| 5 | `fetchWNBAGameBriefFromClaude` (~L31847) | 1 | Single caller only checks `if(text&&textEl)` else removes card — budget-exceeded treated same as any failure. |
| 5 | `computeBroadcastNarrativeIndex` (~L35883) | 7 | All 7 callers do a bare truthy/typeof check on the return value; "not national," "genuinely exciting," and "no BNI type" are all folded into the same null. |
...
| 4 | `fetchLeagueRSS` (~L31004) | 1 | Terminal P4 tier; caller just leaves note null, no further differentiation possible. |
...
| 4 | `fetchGameBriefOnDemand` (~L32289) | 1 | Sole caller (bottom sheet .then) only checks `!text`, removing placeholder regardless of why null. |
...
| 3 | `fetchPrerenderedJournalism` (~L17622) | 1 | Sole caller does `if(relayJournalism?.brief){...}` then falls to client-side AI generation identically for HTTP error, missing brief, or exception. |
...
| 3 | `fetchSchedule` (~L23093) | 4 | Trivial performance.mark() diagnostic wrapper; failure is essentially unreachable and changes nothing downstream. |
...
| 3 | `fetchUserState` (~L29063) | 3 | All three callers (8960, 23329, 29136) invoke fetchUserState() and discard the resolved value entirely. |
...
| 2 | `soccerFBrefInit` (~L8535) | 1 | Sole real caller (line 27950) is pure fire-and-forget — the return value (null or data) is never even read there. |
...
| 2 | `fetchBDLMilestones` (~L30078) | 1 | Sole caller uses `if(ms) note=...` in a fallback chain, discarding why milestones were unavailable. |
| 2 | `fetchNHLStandingsParsed` (~L30115) | 1 | Same fetchStandingsForPrompt discard-the-value pattern as the MLB/NBA standings fetchers. |
| 2 | `fetchMLSStandingsParsed` (~L30140) | 1 | Same fetchStandingsForPrompt discard-the-value pattern. |
| 2 | `fetchStandingsForPrompt` (~L30161) | 3 | All three real callers use Promise.allSettled or .catch(()=>{}), fully discarding the return value. |
| 2 | `getESPNInjuriesForGame(nameToAbbr)` (~L30466) | 2 | Used via `game._homeAbbr \|\| nameToAbbr(...)`; `\|\|` discards why the lookup failed. |
| 2 | `espnInjuriesPrefetch` (~L30495) | 1 | Fire-and-forget from setTimeout; return of fetchESPNInjuries is never read by anyone. |
...
| 1 | `visibilitychange listener (peak_missed)` (~L29041) | 1 | Event handler swallow with no consumer; wrapping adds telemetry only — comments cite a real prior silent-failure bug. |
| 1 | `_connect` (~L29459) | 3 | None of the 3 invocation sites consume a return value; wrapping construction failure adds telemetry only, no behavior change. |
| 1 | `fetchNBAStandingsParsed` (~L30010) | 1 | Same fetchStandingsForPrompt discard-the-value pattern after all three fallback sources fail. |
| 1 | `nhlPlayoffLeadersPrefetch` (~L30639) | 1 | Fire-and-forget setTimeout entrypoint; return value of fetchNHLPlayoffLeaders unused. |
| 1 | `nbaPlayoffLeadersPrefetch` (~L30795) | 1 | Fire-and-forget setTimeout entrypoint; return of fetchNBAPlayoffLeaders never read. |
| 1 | `fetchFIELDBriefFromClaude` (~L31236) | 1 | Inline IIFE result is filtered out via `.filter(Boolean)` regardless of success — fully discarded either way. |
...
| 1 | `isRivalGame` (~L39059) | 7 | TDZ-guard catch collapses to same false as genuine no-rival; none of the 7 callers distinguish error from real negative. |
...
| 1 | `openBottomSheet` (~L41553) | 7 | Postgame drama-string catch just leaves _bsPostgameDrama empty; function is void so none of the 7 callers observe any distinction. |
...
| 1 | `saveSnapshot` (~L42535) | 1 | Both invocation paths (visibilitychange handler, beforeunload reference) never inspect any result; fire-and-forget async call. |
...
| 1 | `fetchGameBriefOnDemand` (~L32344) | 1 | Wraps enrichChampionshipFromArchive(), documented as "null-safe, returns ctx unchanged on any failure." |
| 1 | `fetchEPLMatchBriefFromClaude` (~L32453) | 1 | Optional H2H/form context builder; catch just leaves strings blank and execution continues into the prompt. |
