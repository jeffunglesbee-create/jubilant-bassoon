...
Read the real, current `ESPN_GOTD_SCHEDULE`, `parseBroadcasts`, and the unresolved TODO in `parseBroadcasts` about `b.type==='I'` as a possible auto-detection signal. Check ESPN's real, live scoreboard response shape for a genuinely reliable GOTD signal — `type`, or any other field — that would let this be auto-detected rather than manually maintained. Check whether `field-data-today.json`'s own pipeline (referenced in the object's own comment as the intended auto-detection mechanism) still runs, and if so, why its output isn't reaching `ESPN_GOTD_SCHEDULE` — this may be a second, deeper gap (a broken pipeline) rather than the schedule object simply never being extended by hand.
