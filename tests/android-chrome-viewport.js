#!/usr/bin/env node
// tests/android-chrome-viewport.js — Android Chrome Viewport Audit (Appium + ChromeDriver)
//
// Ports assertions from viewport-all.spec.js to real Android Emulator Chrome.
// Requires: Appium running on port 4723 with UiAutomator2 driver installed.
// ENV: DEVICE_ID (P1/P2/P3/T1/T2), ANDROID_DEVICE (emulator device name)
//
// Usage: node tests/android-chrome-viewport.js
...
// ── Assertion definitions (same JS logic as iOS, different wrapper) ─────────

const UNIVERSAL = [
  {
    id: '#1', name: 'no uncaught JS errors',
    fn: async (browser) => {
      const result = await browser.execute(() => window._fieldErrors || []);
  ...
  },
  {
    ...
    fn: async (browser) => {
      const result = await browser.execute(() => {
        // Fixed June 15 2026 — actual filter container ID is #sport-filters,
        // not the legacy .filter-bar class.
...
async function run() {
  ...
  if (IS_TABLET) assertions.push(...TABLET_ONLY);

  console.error(`[android-chrome] Device: ${ANDROID_DEVICE} (${DEVICE_ID}), ${assertions.length} assertions`);
  ...
  try {
    ...
    if (!ready) console.error('[android-chrome] WARNING: _fieldDataReady never became true');
    ...
    console.error(`[android-chrome] FATAL: ${err.message}`);
    const output = { device: ANDROID_DEVICE, deviceId: DEVICE_ID, error: err.message, results: [] };
    console.log(JSON.stringify(output, null, 2));
    return 1;
  } finally {
