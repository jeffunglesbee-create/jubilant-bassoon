#!/usr/bin/env node
// tests/desktop-chrome-viewport.js — Desktop Chrome Viewport Audit (WebDriverIO + chromedriver)
//
// Tests at D1 (1366x768) and D3 (1920x1080). Pairs with the deploy-gate
// Playwright suite — this provides Chrome-engine coverage of the A612
// desktop-layout invariants (wc-mode + journalism-mode full-viewport swap;
// ESSENTIALS/WHOLE FIELD toggle controls ambient panel).
//
// Requires: chromedriver on PATH (e.g. via chromedriver service or apt).
// ENV: DEVICE_ID (D1 or D3)
//
// Usage: DEVICE_ID=D1 node tests/desktop-chrome-viewport.js
...
const DESKTOP = [
  {
    ...
    fn: async (browser) => {
      ...
      // The Playwright spec asserts #17=2 for both because its IS_DESKTOP set
      // is {D1,D3} — but the CSS demonstrably promotes to 3 at 1440+ on real
      // browsers. Matched against the actual CSS contract here.
  ...
  },
  {
    ...
    fn: async (browser) => {
      // Re-click to toggle back. Test assumes #WF1 ran first; if wf-mode is
      // not set, force it on so the toggle returns to ESSENTIALS.
      await browser.execute(() => {
  ...
  },
  {
    ...
    fn: async (browser) => {
      // Ensure clean state; force #wc-nav-link visible (initWCNav only clears
      // inline display:none if wcActive — bypass by clearing inline style).
      await browser.execute(() => {
        document.body.classList.remove('wf-mode', 'wc-mode', 'journalism-mode');
        const link = document.getElementById('wc-nav-link');
        if (link) link.style.display = '';
      });
      // Toggle wc-mode via the public function (avoids click-target hidden
      // states under #wc-nav-link). Same code path the click would take.
      ...
      return {
        pass: state.hasWc && state.wcSectionVisible && state.mainHidden,
        actual: state,
      };
    }
  },
  {
    ...
    fn: async (browser) => {
      ...
      return {
        pass: state.hasJrn && state.jrnVisible && state.mainHidden,
        actual: state,
      };
    }
  },
  ...
  },
  {
    ...
    fn: async (browser) => {
      // Use the PerformanceObserver collected since page load. If FIELD
      // surfaces _fieldCLS, prefer that. Otherwise query the entries directly.
...
async function run() {
  ...
  console.error(`[desktop-chrome] ${DEVICE_ID} (${VP.label}) ${VP.width}x${VP.height}, ${assertions.length} assertions`);
  ...
  try {
    ...
    if (!ready) console.error('[desktop-chrome] WARNING: _fieldDataReady never became true');
    ...
    console.error(`[desktop-chrome] FATAL: ${err.message}`);
    console.log(JSON.stringify({ engine: 'chromium', deviceId: DEVICE_ID, error: err.message, results: [] }, null, 2));
    return 1;
  } finally {
