...
assert('A56 — field_utils.js loaded in index.html',
  ...
  const FEATURE_GUARDS = [
    ...
    ['A74b','drama-arc-amnesty: bottom-sheet Drama Arc section gated on _bsIsFinal (ADR-002 Step 4)', /\$\{\(_bsIsFinal && sparkline\) \? `<div class="bs-section"><div class="bs-section-label">Drama Arc<\/div>/],
    ['A74c','drama-arc-amnesty: isAmnestyState is the gate source in index.html', [/const _bsIsFinal = isAmnestyState\(eData\?\.state\)/, /function isAmnestyState\s*\(state\)\s*\{\s*return state === 'post' \|\| state === 'final';\s*\}/]],
    ['A75', 'series-margins: buildSeriesMarginsDots defined',         /function buildSeriesMarginsDots\s*\(/],
    ['A75b','forward-window: field-data accept gate admits schema 2.0 AND 2.1', [/schema_version === '2\.0' \|\| d\._meta\?\.schema_version === '2\.1'/, /window_dates\.includes\(TODAY_ISO\)/]],
    ['A75c','forward-window: MLB source is keyed on viewingISO, not TODAY_ISO',   /const _mlbForDate = fieldDataForDate\(viewingISO\)\?\.mlb \|\| \[\]/],
    ['A75d','forward-window: scheduleForDate is pure and falls back to day-0 schedules', [/function scheduleForDate\(fileData, iso\)/, /fileData\.schedules_by_date\[iso\]/, /iso === \(fileData\._meta && fileData\._meta\.for_date\)/, /function fieldDataForDate\(iso\) \{ return scheduleForDate\(_fieldDataCache, iso\); \}/]],
    ['A75e','forward-window: ESPN sweep stays the base for forward days (enrich, not replace)', /sec\.sport !== 'Baseball \(MLB\)'/],
    ['A75f','forward-window: mlbRaw is the override list, not a schedule',        /const mlbRaw=\[\];/],
    ['A75g','uplot: fieldChart imported from the shared chart module',        /import \{ fieldChart, destroyChart \} from '\.\.\/utils\/chart\.js'/],
...
assert('A-NFLPART-1 — NFL P3: participation (formation/pressure) wired (fetch + accessor + row)',
  ...
  'NFL participation must be wired: team-participation.json fetch, getTeamParticipation, NFL_PART, Pass pro scout row');
...
// Per RUWT Deep Analysis Rule 95: MODERATE risk from composite arithmetic
...
//
// What remains (MODERATE, Rule 95): ViewingConditions.evaluate() uses composite
...
//
// RUWT compliance note (Rule 95 — now RESOLVED):
...
//   Rule 95 MODERATE risk is now RESOLVED.
...
assert('A-GAPFIX-1 — zero eData.status reads remain (all renamed to eData.state)',
