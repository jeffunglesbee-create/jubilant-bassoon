...
assert('A419 — PM-26-L-1: min-height:180px on base .game-card (absorbs editorial-injection growth)',
  // The min-height: 180px declaration must be present inside the base
  ...
  cardBaseBlock.length > 0 &&
  /min-height:\s*180px/.test(cardBaseBlock),
  'PM-26-L-1 fix. WPT analysis on PM-26-J-1b deploy (sw.js?v=2026-06-03o, run 1) identified three deterministic CLS shifts at ~2.4s (0.363), ~4s (0.566), ~4.8s (0.228) — each timed within ~150ms of network arrivals (ESPN/MLB schedule data, GitHub outbox JSON, relay /field/data/today, ESPN scoreboards, /journalism/generate). Diagnosis: each data arrival triggers a render that injects content into cards (drama badges, score-wrap, scout-pick chips, situation indicators, vibe chips — the Rule-24 trigger chain). Each injection grows the card vertically. Vertical growth cascades through the grid because contain:layout (added in PM-26-J-1) only stops style/paint/internal-layout invalidation — it does NOT stop a card\'s external size changes from affecting siblings. Fix: min-height:180px on base .game-card reserves enough vertical space up-front that most editorial injection stays within the reserved bounds — card stays at 180px floor, neighbors do not shift when content arrives. 180px matches the contain-intrinsic-size value already present (consistent with PM-26-J spec author\'s averaged-card-height estimate). Tradeoff accepted: pre-game cards (typically ~88-100px native content height) will have empty bottom space. Live/final cards (typically 180-220px) already at/above the floor. NOT addressing in this commit: (a) skeleton-to-real transition (skeletons remain at 88px; the 88→180 transition is still a shift but it is single-event not cascading), (b) editorial content that grows beyond 180px (will still shift, but only by the residual delta above floor, not by the full content height). Patent relevance: combined with PM-26-K-1 font fallback overrides, this addresses the two correlated CLS contributors identified in the J-1b WPT trace; the perceived-perf claim narrative becomes "containment + reserved space + matched-metric fallbacks" — a coherent multi-layer architecture defense.');
