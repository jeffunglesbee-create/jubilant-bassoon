#!/usr/bin/env python3
"""
FBref Soccer Squad Stats → R2 (MLS only)

Fetches squad-level xG/pressing/passing/GK analytics from FBref for MLS 2025.
Output: R2 soccer/fbref/mls.json (consumed by context-assembler 'mls' lookup)

Cloned from soccer-fbref-wc.py with single-league config. Cron resumes July 20
2026 — first Monday after the FIFA WC final (July 19). Off-season runs return
empty tables; upload still proceeds so the key never goes missing.
"""
